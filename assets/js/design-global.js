 ...
 */
 ...
 $(document).ready(function() {
  ...
  function  set_header()
  {
    ...
    $(".panneau-introduction-texte > .row > .columns > *:not(:first-child, br)").wrapAll("<div id='second_text' style='max-width : 900px'></div>");
    ...
    $(".panneau-introduction").append('<div id="third_text" class="small-12 columns text-center" style="position: absolute; bottom: 0; left: 0; padding-bottom: 20px;"><div class="row"><div class="small-centered columns"><a href="#slide-0"><p class="text_bold" style="line-height: 0;">JE RÉPONDS</p><img src="https://s3.amazonaws.com/heroku-adfinitas-campaign/ESF-Donor-Journey-2016/arrow_down.png"></a></div></div>');
    vertical_center();
  }
  ...
  isValidForm = function(jqueryFormSelector) {
    var status = true;
    var check = false;
    ...
    if (status == false && check_parent($(this)) == true)
     $("#slide-" + (slide_count - 1) + "> .row > .columns > .columns").first().prepend("<p class='see_above error-message-highlight' style='padding-top: 5px; padding-bottom: 5px'>⬆ Certaines questions requises n'ont pas étés remplies</p>");
   return (status);
 }
 ...
 function  vertical_center()
 {
  ...
  console.log($(".panneau-introduction").first().height());
  space = ($(".panneau-introduction").first().height() - ($("#first_text").offset().top + $("#second_text").offset().top)) / 2;
  space -= $(".panneau-introduction-texte > .row > .small-6").first().height() / 2;
...
function    move_error()
{
  ...
  $(".error-message").addClass("error-message-highlight");
}
...
function    apply_last_slide()
{
  ...
  $("#slide-" + place + "> .field-row > .columns > .container-all-radio > *").last().removeClass("small-6");
  $("#slide-" + place + "> .field-row > .columns > .container-all-radio > *").last().wrap( "<div class='columns'></div>" );
...
function    hide_fields()
{
    ...
    while (count < text.length)
    {
        if ($("input[name='" + text[count] + "']").length == 1) {
            $("input[name='" + hide[count] + "']").parent().remove();
            $("select[name='" + hide[count] + "']").parent().parent().remove();
        }
        count++;
    }
